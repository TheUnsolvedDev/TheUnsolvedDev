...
async function init() {
  try {
    ...
    $('#research-grid').innerHTML = data.highlights.map((h,i) => `<article class="card"><div class="card-top"><span>PROFILE HIGHLIGHT</span><span>${String(i+1).padStart(2,'0')} / RESEARCH</span></div><h3>${escapeHTML(h.name)}</h3><p>${escapeHTML(h.description)}</p><div class="tags">${h.tags.map(t=>`<span class="tag">${escapeHTML(t)}</span>`).join('')}</div><a class="text-link" href="${escapeHTML(h.url)}">Read the source documentation ↗</a></article>`).join('');
